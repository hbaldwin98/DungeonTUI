...
func (m *Model) ensureBrowserSelection() {
	if m.usesCampaignTree() {
		if m.navKind == "" {
			chosen := 0
			for index, entry := range m.navEntries() {
				if entry.Kind == NavType && entry.Type == domain.NPC {
					chosen = index
					break
				}
			}
...
func (m Model) renderBrowserLeaf(pane prefs.Pane, width, height, originX, originY int, regions *[]browserRegion) string {
	...
	case prefs.PaneDetail:
		...
		content, lines := m.detailBodyView(innerWidth)
		var used int
		body, used = windowLinesFromLines(content, lines, innerHeight, scroll)
		if view != nil && view.key == m.detailSelectionKey() {
			view.offset = used
		}
		if regions != nil {
